...
static int parse_options(int argc, char **argv)
{
  int c;
    ...
    if ((ksuseopc_addr != NULL) &&  (ksusetim_addr != NULL)) {
        fprintf(stderr, "Oracle userspace tracing: ksuseopc_addr = %lu, ksusetim_addr = %lu\n", ksuseopc_addr, ksusetim_addr);
    } else {    
        fprintf(stderr, "No Oracle userspace tracing: ksuseopc_addr = 0, ksusetim_addr = 0\n");
    }
    printf("\n");
...
int main(int argc, char **argv)
{
    ...
    fprintf(stderr, "\nptrace_profiler: a userspace and kernel stack profiler extended with Oracle wait event data collection\n");
